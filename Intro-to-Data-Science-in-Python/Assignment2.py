...
def answer_two():
    ...
    return df.loc[diff.idxmax()].name
answer_two()
'United States'
Question 3
...
census_df['STNAME'].value_counts().argmax()
def answer_three():
...
def answer_five():
    dt = census_df.groupby('STNAME').agg({'CTYNAME':'count'})
    return dt.loc[dt['CTYNAME'].idxmax()].name
answer_five()
'Texas'
Question 6
...
state's name
def answer_six():
    dt = census_df.groupby(['STNAME','CTYNAME']).agg({'CENSUS2010POP': sum})
    dt_group = dt['CENSUS2010POP'].groupby(level=0, group_keys = False)
    df_sort = dt_group.apply(lambda x: x.order(ascending = False).head(4))
    df_sort = pd.DataFrame(df_sort).reset_index()
    cd = df_sort[df_sort['STNAME']!=df_sort['CTYNAME']]  #Remove state's name
    df_2 = cd.groupby(['STNAME']).agg({'CENSUS2010POP': sum}).reset_index().sort_values(by='CENSUS2010POP',ascending=False)
    return df_2.head(3)['STNAME'].tolist()
answer_six()
...
def answer_seven():
    ...
    ab = Max - Min
    census_df['ab'] = ab
    cd = census_df[census_df['STNAME']!=census_df['CTYNAME']]
    ct = cd.loc[cd['ab'].idxmax()]['CTYNAME']
    return ct
answer_seven()
'Harris County'
Question 8
